...
import {BleepsAccountData} from '$lib/account/account-data';
...
const logger = logs('bleeps');

export const accountData = new BleepsAccountData();
...
// But note that Bleeps is designed to work without external RPC
				...
				if (!signature) {
					async function signMessage() {
						const msg = stringToHex(
							'Welcome to Bleeps, Please sign this message only on trusted frontend. This gives access to your local data that you are supposed to keep secret.',
						);
