...
describe('The layout height shell', () => {
	...
	test('every inherited route renders into the region', async ({page}) => {
		...
		// `/bleeps/` stands in for the template's `/demo/`, which this app replaced.
		// Same substitution overlays.e2e.ts already makes: pointing an inherited
		// suite at a route Bleeps does not have does not fail, it asserts against
		// the 404 page and passes, which is how one of these went unnoticed before.
		for (const path of ['/', '/bleeps/', '/transactions/', '/explorer/']) {
