...
// from https://github.com/paulrberg/prb-proxy/blob/main/contracts/PRBProxy.sol
// with modifications:
// - ownership management is externalized
// - no hardcoded first owner, use constructor
...
error PRBProxy__NotAdmin(address admin, address caller);
...
contract PRBProxy is IPRBProxy {
    // TODO event AdminSet(address indexed newAdmin);
    ...
    /// @notice The address of the admin account or contract, which is the only account able to change the owner.
    ...
    constructor(address initialOwner, address initialAdmin) {
        ...
        owner = initialOwner;
        admin = initialAdmin;
        emit TransferOwnership(address(0), initialOwner);
    }
    ...
    function transferOwnership(address newOwner) external override {
        if (admin != msg.sender) {
            revert PRBProxy__NotAdmin(admin, msg.sender);
        }
        owner = newOwner;
